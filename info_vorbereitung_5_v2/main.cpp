...
int check_winner(const char grid[]) {
    /*for (int x = 0; x < 3; x++) {
        for (int y = 0; y < 3; y++) {
...
int main() {
    ...
    cout << "Bitte geben sie x, y getrennt durch Leerzeichen ein." << endl;
    int moves = 0;
    char player = 'A';
    bool running = true;
    while (running) {
        ...
        while (!input_correct) {
            ...
            cin >> y_in >> x_in;
            if (get_sign(grid, y_in - 1, x_in - 1) == 2)
                input_correct = true;
            else
                cout << "Eingabe fehlerhaft. Bitte versuchen sie es erneut." << endl;
        }
